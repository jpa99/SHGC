...
% Import Optical Spectra
 ...
 %% PLoting Optical Traces
    ...
    % Calculating Tlum for ON State
            ...
            % Calculate from 380 to 400
            Tlumprod_ON(4,1)=onData(31,2)*EyeLum(1);
            Tlumprod_ON(5,1)=onData(41,2)*EyeLum(2);
            
            %Calculate Tlum from range of 400-770
            g=41;
            gg=91;
            for j=6:43
                Tlumprod_ON(j,1)=onData(g+10,2)*EyeLum(j-3);
                g=g+10;
                gg=gg+10;
            end
    ...
    % Calculating Tlum for OFF State
            ...
            % Calculate from 380 to 400
            Tlumprod_OFF(4,1)=offData(31,2)*EyeLum(1);
            Tlumprod_OFF(5,1)=offData(41,2)*EyeLum(2);
            
            %Calculate Tlum from range of 400-770
            g=41;
            gg=91;
            for j=6:43
                Tlumprod_OFF(j,1)=offData((g+10),2)*EyeLum(j-3);
                g=g+10;
                gg=gg+10;
            end
